...
const Contact = () => {
  return (
    <section>
      ...
      <section className="flex flex-col md:flex-row-reverse md:items-center md:justify-center">
        <div className="formBox  px-7 py-15 flex flex-col items-start gap-4 md:w-[50%] ">
          <h2 className=" text-[1.8rem] md:text-[2.6rem] font-bold ">
            Reach Out to Us
          </h2>
        ...
        <section className="contact-cards  px-7 py-15 flex flex-col md:items-end gap-6 md:w-[40%] ">
          {contactCards.map((card) => (
            <Link
              to={card.refLink}
              key={card.title}
              className="contact-card w-full max-w-[450px] md:max-w-[360px] py-[3rem]  flex items-center gap-4 p-4 border border-gray-300 hover:border-chart-4 rounded-md hover:shadow-lg transition duration-300"
            >
              <span className="icon text-[2rem] text-chart-4">{card.icon}</span>
              <div className="text">
